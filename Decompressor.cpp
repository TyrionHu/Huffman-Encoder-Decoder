...
int main(int argc, char * argv[])
{
    ...
    file_count += 256 * process_8_bits_NUMBER(current_byte, current_bit_count, fp_compressed);
        // File count was written to the compressed file from least significant byte 
        // to most significant byte to make sure system's endianness
    ...
    for(int current_file = 0; current_file < file_count; current_file++)
    {
        ...
        if(this_is_a_file(current_byte, current_bit_count, fp_compressed) == true)
        {   
            ...
            char new_file[file_name_length + 4];
            write_file_name(new_file,file_name_length, current_byte, current_bit_count, root, fp_compressed);
            change_name_if_exists(new_file);
            //--------------------------------------------------

            translate_file(new_file, size, current_byte, current_bit_count, root, fp_compressed); //translates .eighth
        }
        //if this is a folder
        else
        {   
...
{
    unsigned char value, temp_byte;
    fread(&temp_byte, 1, 1, fp_read);
    value = current_byte | (temp_byte >> current_bit_count);
...
long int read_file_size(unsigned char & current_byte, int current_bit_count, FILE * fp_compressed)
{
    ...
    return size;
    // Size was written to the compressed file from least significant byte 
